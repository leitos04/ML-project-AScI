...
for epoch in range(epochs):
    ...
    validation_loss = compute_accuracy(val_loader, net)

    for loss in training_loss:
        training_loss_list.append((epoch + 1, loss))

    for loss in validation_loss:
        validation_loss_list.append((epoch + 1, loss))

    with open("training_loss_values.txt", "w") as file:
    	for epoch, loss in training_loss_list:
        	file.write(f"{epoch}\t{loss:.4f}\n")

    with open("validation_loss_values.txt", "w") as file:
    	for epoch, loss in validation_loss_list:
        	file.write(f"{epoch}\t{loss:.4f}\n")
print("DONE")
